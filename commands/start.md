---
description: Boot a Shiroe session. Runs interview if config/PROJECT.md missing, otherwise restores context from the flat memory/ layout per AGENTS.md §"First action every session".
---
...
3. If user cancels mid-interview: Shiroe boots in READ-ONLY mode until the schema is complete (per `config/PERMISSIONS.md`).

Otherwise (normal boot, reading order per AGENTS.md §"First action every session"):
