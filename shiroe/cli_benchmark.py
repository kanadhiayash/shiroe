"""CLI subcommands for ``shiroe benchmark``.

Split from ``shiroe.cli`` to keep that module from ballooning further, same
pattern as ``shiroe.cli_capability`` / ``shiroe.cli_providers``. Registered via
``register(sub)`` and ``handle(args)``.

``benchmarks/external`` is dev/test-only (never packaged under ``shiroe*`` —
...
def register(sub) -> None:
    ...
    ext = bsub.add_parser(
        "external",
        help="Run LoCoMo/LongMemEval/ConvoMem/... across the shiroe, full_context, and bm25 arms",
    )
    ...
    ext.add_argument("--arms", default="all",
                     help="comma-separated arm names, or 'all' (shiroe,full_context,bm25)")
...
def _cmd_external(args: argparse.Namespace) -> int:
    try:
        ...
        _error(
            ...
            "Run this command from a source checkout of shiroe, "
            "not from an installed shiroe package."
        )
        return 1
