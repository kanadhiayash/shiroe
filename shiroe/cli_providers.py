"""CLI subcommands for ``shiroe providers``.

Split from ``shiroe.cli`` to keep that module from ballooning further, same
pattern as ``shiroe.cli_capability``. Registered via ``register(sub)`` and
``handle(args)``.
"""
...
from shiroe.adapters.providers import available_providers
from shiroe.security import ConnectorDisabledError, NetworkDeniedError


def register(sub) -> None:
    p = sub.add_parser("providers", help="Provider model registry (SHR-60)")
...
def handle(args: argparse.Namespace) -> int:
    if args.providers_command == "refresh":
        from shiroe.adapters.providers.refresh import refresh_provider
