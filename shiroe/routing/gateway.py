...
from shiroe.adapters.providers import DEFAULT_PROVIDER, resolve_model
...
from shiroe.routing.criticality import Classification, TaskSignals, classify

POLICY_VERSION = "shiroe.routing.gateway/v1"
...
class ApprovalRequiredError(RoutingError):
    ...
    SHR-61: an ambiguous classification is not auto-routed just because it
...
class ModelCallRequest:
    ...
    # SHR-60: a dry-run never calls the model — it's a validation/local-check
...
class RouteDecision:
    ...
    # SHR-61: populated only when the criticality came from classify_and_route
    # rather than being handed in already-validated. None means "the caller
    # asserted this criticality directly" — the pre-SHR-61 path, unchanged.
...
def route(request: ModelCallRequest) -> RouteDecision:
    ...
    # SHR-60: capability fail-closed check happens inside resolve_model,
...
def classify_and_route(
    ...
    *,
    ...
    """SHR-61: classify ``signals`` and route, instead of trusting a caller-typed criticality.
    ...
    (:mod:`shiroe.routing.criticality`) does it from structured signals and
