...
Every prompt Shiroe emits lands **after** a target model's own system prompt — which the target has already ingested and cached. If Shiroe knows what the target already knows, the `caveman-handoff` + `prompt-context-engine` skills can drop redundant content (tool declarations, format instructions, refusal patterns, persona shifts, constraint restatements) and reclaim 20-40% of the prompt budget for actual task content.

The reverse is equally load-bearing: knowing each target's refusal signature and format quirks lets Shiroe pre-shape prompts to match the target's attention biases, avoiding re-prompts and one-shotting more requests.

## Boundary

- **No source text vendored.** Shiroe imports **derived observations** only, into `references/target-model-profiles/<target_id>.md` — one 200-400 token YAML per target.
- Original prompts remain the model vendors' IP. Shiroe stores summaries + tokens counts + format quirks + refusal signatures, not text.
...
- Copying prompt bodies into this repo, `memory/`, `docs/`, or any Shiroe-canonical surface.
...
- Using the leaks catalog as an oracle for legal, safety, or policy claims about any vendor. Shiroe treats every profile row as a routing hint, not a fact about the vendor.

## Refresh workflow

1. Monthly, or on any Shiroe target-router change.
