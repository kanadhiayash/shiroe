"""SHIROE_* env vars, with the pre-rebrand ZEREF_* names still honoured.

Environment variables live in shell profiles, CI configs, and systemd units
this repo cannot rewrite. Dropping ZEREF_* outright would not fail loudly --
an unset variable falls back to its default, so a network guard would
