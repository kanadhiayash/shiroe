"""``zeref providers refresh`` — partial, honest self-update (ZRF-60 PART C).
...
``zeref lineage import`` (github) and ``zeref grade --llm`` (litellm) — see
``zeref/security/policy.py``. OFF by default, so it is OFF in CI and never
...
from zeref.adapters.providers.base import PROVIDER_SCHEMA_V2
