"""External benchmark truth gate (ZRF-66 / issue #172).
...
def build_capability_matrix(root: Path) -> list[CapabilityEvidence]:
    ...
    zeref-registry.json cross-referenced with tests/; the routing classifier
...
def scan_public_claims(root: Path) -> list[ClaimFinding]:
    ...
    for path in _public_docs(root):
        ...
        for lineno, line in enumerate(text.splitlines(), start=1):
            ...
            if "zeref" in lowered and _SCORE_RE.search(line) and (
